macroScript BipedKeyKeyKey
	category:"Bomggotge"
	toolTip:"BipedKeyKey Enhanced"
	icon:#("VCRControls", 17)
(
	...
	(
	    ...
	    (
	        ...
	        (
	            ...
	            (
	                ...
	                (
	                    ...
	                    ( 
							isAnyKeySelected = true
	                        append SelectedkeyList[i] (getKeyTime ctrls[i] k)
	                    )
	                )
	            )
	        )
			if not isAnyKeySelected then
			(
				for i = 1 to 3 do
				(
					for k = 1 to ctrls[i].keys.count do
					(
						append SelectedkeyList[i] (getKeyTime ctrls[i] k)
					)
				)
			...
			if not isAnyKeySelected then
			(
				for k = 1 to s.controller.keys.count do
				(
					append SelectedkeyList (getKeyTime s.controller k)
				)
			)
